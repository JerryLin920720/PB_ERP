...
// ─────────────────────────────────────────────────────────────────
function Ba025ContactsTab({ rows, isEditing, loading, onCellChange, onAddRow, onDeleteRow, activeRecord }) {
  const handleAdd = () => {
    onAddRow({
...
// ─────────────────────────────────────────────────────────────────
function Ba025MasterForm({ form, isEditing, activeRecord, updateMasterField }) {
        ...
        <Row gutter={8}>
          <Col span={3}>
            <Form.Item
              name="factno"
              label="材料商編號"
              rules={[{ required: true, message: '材料商編號為必填' }]}
        ...
        <Row gutter={8}>
          ...
          <Col span={4}>
            <Form.Item
              name="ba020gkey"
              label="材料分類"
              rules={[{ required: true, message: '材料分類為必填' }]}
              style={{ marginBottom: 4 }}
            >
              <Select
                showSearch
...
// ─────────────────────────────────────────────────────────────────
export const Ba025V2Sheet = createRecordWorkbenchSheet({
  sheetId: 'ba025',
  title: '材料商基本資料管理',
  breadcrumb: ['基礎資料', '材料商基本資料管理'],

  api: {
    listUrl: `${API}ba015/?type=2`,
    deepSaveUrl: `${API}ba015/deep_save/?type=2`,
    deleteUrl: `${API}ba015/`,
  },
  ...
  renderQueryForm: ({ form: qForm }) => (
    <Row gutter={8}>
      <Col span={5}>
        <Form.Item name="factno" label="材料商編號">
          <Input placeholder="輸入材料商編號搜尋..." />
        </Form.Item>
      </Col>
      <Col span={5}>
      ...
      <Col span={5}>
        <Form.Item name="ba020gkey" label="材料分類">
          <CategoryQuerySelect placeholder="選擇材料分類搜尋..." allowClear />
        </Form.Item>
      </Col>
      <Col span={5}>
  ...
  renderMasterForm: (props) => <Ba025MasterForm {...props} />,
  ...
  buildDeepSavePayload: (master, detailStates) => {
    const contacts = (detailStates.contacts?.rows || [])
      ...
      .map((c) => ({
        ...
        parentgkey: null, // No branches exist for BA025
      }));
