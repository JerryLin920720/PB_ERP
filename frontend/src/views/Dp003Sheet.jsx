...
const API_URL = 'http://localhost:8001/api/dp003/';

/**
 * 👞 DP003 鞋種類別設定 (Shoe Type Setup)
 * 物理對齊: w_dp003.srw / d_dp003.srd
 */
export default function Dp003Sheet() {
  ...
  const handleSave = async () => {
    // 物理校驗：必填檢查
    const invalid = entities.find(e => !e.shoetype || !e.eshoetype);
    if (invalid) {
      message.error('中文與英文名稱皆為必填項目');
      return;
    }
            ...
            { 
              ...
              render: (v, r) => (
                <Input 
                  variant="borderless"
                  value={v} 
                  onChange={e => handleUpdate(r.gkey, 'shoetype', e.target.value)} 
                  placeholder="輸入中文名稱..."
            ...
            { 
              ...
              render: (v, r) => (
                <Input 
                  variant="borderless"
                  value={v} 
                  onChange={e => handleUpdate(r.gkey, 'eshoetype', e.target.value)} 
                  placeholder="輸入英文名稱..."
                />
              )
            }
