...
const { Option } = Select;
const API_URL = 'http://localhost:8001/api/dp005/';
...
export default function Dp005Sheet() {
  ...
  const handleSave = async () => {
    ...
    const invalid = entities.find(e => !e.partgroup || !e.epartgroup);
    if (invalid) {
      message.error('名稱為必填項目');
      return;
    }
  ...
  const partTypeOptions = [
    'UPPER', 'UPPER/LINING', 'LINING/SOCK', 'SOCK/OUTSOLE', 
    'OUTSOLE/HEEL', 'HEEL/TONGUE', 'TONGUE/ORNAMENT', 'ORNAMENT/OTHER', 'OTHER'
  ];
  ...
  const columns = [
      ...
      render: (v, r) => (
        isEditing ? (
          <Input 
            variant="borderless"
            value={v} 
            onChange={e => handleUpdate(r.gkey, 'partgroup', e.target.value)} 
            style={{ fontSize: '13px' }}
          />
    ...
    { 
      title: '類別名稱 (英文)', 
      dataIndex: 'epartgroup', 
      render: (v, r) => (
        isEditing ? (
          <Input 
            variant="borderless"
            value={v} 
            onChange={e => handleUpdate(r.gkey, 'epartgroup', e.target.value)} 
            style={{ fontSize: '13px' }}
          />
        ) : <Text style={{ fontSize: '13px', paddingLeft: '11px' }}>{v}</Text>
      )
      ...
      render: (v, r) => (
        isEditing ? (
          <Select 
            variant="borderless"
            value={v} 
            onChange={val => handleUpdate(r.gkey, 'parttype', val)}
            style={{ width: '100%', fontSize: '13px' }}
          >
            {partTypeOptions.map(opt => <Option key={opt} value={opt}>{opt}</Option>)}
          </Select>
        ) : <Text style={{ fontSize: '13px', paddingLeft: '11px' }}>{v}</Text>
      )
    }
  ...
  return (
    <div style={{ padding: '15px', backgroundColor: '#f0f2f5', height: '100vh', display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <style>{`
        ...
        .ant-input-borderless:focus, .ant-input-borderless-focused { background-color: #fff; }
      `}</style>
    </div>
  );
}
