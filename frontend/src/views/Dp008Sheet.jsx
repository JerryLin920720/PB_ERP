...
const API_URL = 'http://localhost:8001/api/dp008/';
...
export default function Dp008Sheet() {
  ...
  const handleSave = async () => {
    ...
    const invalid = entities.find(e => !e.socklabel);
    if (invalid) {
      message.error('Sock Label 為必填項目');
      return;
    }
      ...
      render: (v, r) => (
        isEditing ? (
          <Input 
            variant="borderless"
            value={v} 
            onChange={e => handleUpdate(r.gkey, 'socklabel', e.target.value)} 
    ...
    <div style={{ padding: '15px', backgroundColor: '#f0f2f5', height: '100vh', display: 'flex', flexDirection: 'column', gap: '15px' }}>
      <div style={{ backgroundColor: '#fff', padding: '8px 15px', borderRadius: '4px', border: '1px solid #d9d9d9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Space>
          ...
          <Text strong>DP008 Sock Label 設定</Text>
